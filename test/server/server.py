...
def event_callback(job_template):
    ...
    assert 'callback_url' in flask.request.json, \
        'callback_url not provided'

    job_id = random.randint(1,10000000)
    if job_template.startswith('deploy'):
        add_deploy_job(job_template, flask.request.json['callback_url'], job_id)
    elif job_template.startswith('destroy'):
        add_destroy_job(job_template, flask.request.json['callback_url'], job_id)
