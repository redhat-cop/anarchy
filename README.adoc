...
+
----
oc start-build -n anarchy-operator anarchy-operator --from-dir=.
----

. Start the ansible-runner build:
+
----
oc start-build -n anarchy-operator anarchy-runner --from-dir=.
----
...
----
INGRESS_DOMAIN=$(oc get ingresses.config.openshift.io cluster -o jsonpath='{.spec.domain}')
OPERATOR_IMAGE=$(oc get imagestream anarchy-operator -n anarchy-operator -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}')
RUNNER_IMAGE=$(oc get imagestream anarchy-runner -n anarchy-operator -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}')
oc process \
  -f deploy-template.yaml \
  -p OPERATOR_HOSTNAME=anarchy-operator.$INGRESS_DOMAIN \
...
+
----
ansible-playbook test/test-playbook.yaml
----
