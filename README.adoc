...
. Build the anarchy-operator image:
+
----
oc start-build -wF anarchy-operator --from-dir=.
----
...
----
INGRESS_DOMAIN=$(oc get ingresses.config.openshift.io cluster -o jsonpath='{.spec.domain}')
OPERATOR_IMAGE=$(oc get imagestream anarchy-operator -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}')
